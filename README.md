...
- **辨識方式可選（手機內建 AI / Azure AI Foundry）**：「辨識：…」按鈕可以切換。
  ...
  - 兩種辨識器都實作同一個 `Recognizer` 介面，所以切換辨識方式不需要動任何
    呼叫端的程式碼
  - **成本保護**：即時模式每出現一個新的追蹤 ID 就會觸發一次辨識，手機內建
    AI 免費所以無所謂，但換成 Azure 的話拿著手機走一圈可能就是幾十次付費呼叫。
    所以 Azure 模式下即時模式**不自動辨識**，改成點灰框才送（物件穩定時的
    清晰裁切圖會先存著，點的時候直接用）。另外有每分鐘 20 次的限流當安全網，
    防的是程式邏輯出錯狂打 API，正常手動操作碰不到這個上限；限流器存在畫面層，
    不會因為改設定而歸零
...
- 限流與成本保護（`RateLimiterTest`）：滑動視窗、被拒絕的呼叫不佔額度、
  Azure 模式不會自動辨識
