...
- **App icon**：`res/drawable/ic_launcher_background.xml`（深藍底）＋
  ...
  再另外準備舊版密度分層的 PNG 圖示
- **VR cardboard 模式**：「VR 模式」按鈕會把畫面鎖成橫向、切成左右兩半
  （塞進 cardboard 這類 VR 眼鏡用）。手機放進眼鏡後摸不到螢幕，所以互動靠
  畫面中央固定的準星（代表「頭現在指向哪裡」）：
  - 凝視停留 1.5 秒（綠色準星）：選取——已命名的唸出來，還沒辨識的送辨識
  - 繼續停留到累積 3 秒（準星變橘色）：標記／取消標記單字，有語音確認
    （「已加入單字本」）。一般模式的長按是開命名對話框，但那在 VR 裡行不通：
    對話框整個螢幕置中，戴著眼鏡時每隻眼睛只看得到一半，而且要打字。長按
    功能裡唯一不用打字的就是標記，所以 VR 裡只做這件事；改名、查字典之後在
    一般模式從單字本處理
  - 眼鏡側邊的按鈕（很多 cardboard 有，按下去會碰觸螢幕）：立刻選取，不用等
  - 手機拿出來後長按螢幕或按返回鍵：退出 VR
  - 控制列在 VR 裡不顯示（它會橫跨兩眼），提示文字畫在左右各自的畫面裡；
    系統的狀態列、導覽列也會隱藏（全螢幕沉浸模式），從邊緣滑動可以暫時叫出來

  - **兩套畫面引擎**（在「設定」裡切換）：
    - **流暢（預設）**：OpenGL。相機預覽直接送進 GPU，畫面跟相機一樣順，
      不再是每秒 6～7 張的分析影格——在 VR 裡轉頭時畫面又慢又延遲，是暈眩的
      典型原因。每一格先畫出「一隻眼睛的畫面」（相機＋框＋準星）到離屏貼圖，
      再貼到左右兩眼、各自以鏡片中心做變形校正。手機只有一顆後鏡頭，兩眼
      內容本來就一樣，所以只畫一次、貼兩次。框和文字也畫進同一張貼圖、跟相機
      影像一起變形，透過鏡片才會對齊（如果框畫在 GL 外面，越靠邊偏越多）。
      也不用每格把影像轉成 Bitmap，比舊版省電
    - **相容**：舊版畫面，用分析影格、沒有變形校正，但最穩定。新引擎的退路
  - **鏡片間距、變形校正可調**：每款 cardboard 的鏡片都不一樣，沒有對所有
    眼鏡都對的數字，只能戴上去調。間距調到兩眼畫面合成一個、不重影；變形
    看靠近邊緣的直線，往中心凹（校正不夠）就調大、往外凸（校正過頭）就調小
  - **流暢引擎只驗證過「編譯得過」**：OpenGL 的錯誤多半是執行時才看得到
    （黑畫面、上下顛倒），CI 抓不到。所以做了兩層保險：啟動失敗（例如某支
    手機的 GPU 不支援某個語法）會自動退回相容模式、不會閃退；畫面怪怪的
    也可以在設定裡手動切回相容。VR 模式鎖橫向，相機只會需要轉 0 或 180 度，
    這兩種都有處理；理論上不會出現的 90／270 度沒去猜取樣方向，只記錄 log
  - 一顆後鏡頭，兩眼看到同一張圖，不會有真正的立體深度
...
- **轉橫向會跳出 VR、設定會遺失**：App 沒處理螢幕方向，轉向時 Android 會
  重建畫面，而模式和設定都只存在記憶體裡。結果是進 VR 要把手機轉橫、一轉就
  跳回一般模式；一般模式轉一下手機，顯示模式、翻譯語言、更新頻率也會全部
  回到預設值。現在 VR 鎖橫向、其他模式鎖直向，模式和設定改用
  `rememberSaveable`，畫面重建也保得住。照片（Bitmap）存不進重建前的狀態，
  如果還原回拍照模式卻沒有照片，會退回一般模式
- **VR 流暢模式畫面轉了 90 度、還被壓扁**（實機回報）：相機畫面直接送進
  SurfaceTexture 時，相機會把「感光元件的方向」寫進去，SurfaceTexture 的貼圖
  矩陣已經先把畫面轉成手機「自然方向」（直向）的正向。官方文件
  `SurfaceRequest.TransformationInfo` 寫得很清楚：這種情況（`hasCameraTransform()`
  為 true）只需要補「螢幕目前轉了多少」，其他情況才用 `getRotationDegrees()`。
  原本的程式用了後者，橫向時補轉 0 度，於是顯示的是直向的畫面：轉了 90 度，
  而且直向畫面被硬塞進橫向的裁切範圍，比例也被壓扁（截圖裡的地磚看起來斜斜的
  長菱形就是這個原因）。現在依 `hasCameraTransform()` 分兩種情況處理，旋轉方向
  的推導寫在 `VrMath.cameraQuarterTurnsCcw` 的註解裡，也寫成了測試。
  另外 VR 改成固定一個橫向（原本兩個橫向都可以）：兩個橫向之間翻轉 180 度時
  Android 不會重建畫面，相機的方向資訊不會更新，框會整個上下顛倒
- **VR 畫面上疊著系統列**（實機截圖發現）：時間、通知圖示、電量和底部的手勢
  橫條都還疊在 VR 畫面上，橫跨兩眼。現在 VR 模式會進入全螢幕沉浸模式
- **VR 模式螢幕會自己暗掉**：VR 靠凝視操作、不碰螢幕，系統的自動關閉螢幕
  計時器照跑，戴到一半會變黑。現在相機畫面開著就保持螢幕常亮
- **進出 VR 會重查更新**：進出 VR 會因為鎖定方向而讓畫面重建，更新檢查原本
  每次建立都會跑，有新版時對話框還會一直跳出來蓋住 VR 畫面。改成每次啟動
  App 只查一次
...
- 失敗原因（`HttpStatusMappingTest`、`LabelStateTest`）：HTTP 狀態碼對應、
  ...
  的回歸防護
- VR 渲染幾何（`VrMathTest`）：GPU 上的 shader 測不到，但它跟這裡用同一套
  公式——FILL_CENTER 裁切、鏡片中心位置、變形校正的取樣位置（鏡片中心永遠
  對準畫面中心、校正越強推得越外面、在非正方形畫面上是圓形而不是橢圓），
  以及相機畫面的旋轉補正（把「橫拿時天空要回到畫面上方」的物理推導寫成測試，
  旋轉方向如果被改反會直接失敗）
